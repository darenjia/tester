...
def create_spec_file(onefile=True):
    ...
    # 转义路径中的特殊字符
    project_root_path = project_root.replace(os.sep, '/')
    app_path = os.path.join(project_root, 'app.py').replace(os.sep, '/')
    template_path = os.path.join(project_root, 'web/templates').replace(os.sep, '/')
    static_path = os.path.join(project_root, 'web/static').replace(os.sep, '/')
    icon_path = os.path.join(project_root, 'build/icon.ico').replace(os.sep, '/')
    ...
    # 构建version参数行
    version_file_path = os.path.join(project_root, 'build', 'version.txt').replace(os.sep, '/')
    version_line = f'    version=r"{version_file_path}",'
    
    # onefile配置
    onefile_config = 'True' if onefile else 'False'
...
def build_executable(onefile=True):
    ...
    # 创建 spec 文件
    print("\n[3/5] 创建打包配置...")
    spec_file = create_spec_file(onefile=onefile)
