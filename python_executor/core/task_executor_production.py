...
class TaskExecutorProduction:
    ...
    def _load_configuration(self, task: Task | ExecutionPlan):
        ...
        try:
            result = self.controller.load_configuration(config_path)
            if result:
                logger.info("配置文件加载成功")
                self.current_collector.add_log("INFO", "配置文件加载成功")
            else:
                raise ToolException(f"配置文件加载失败: {self._controller_last_error()}")
    ...
    def _load_configuration_by_path(self, config_path: str):
        ...
        try:
            configuration_capability = None
            if hasattr(self.controller, "get_capability"):
                configuration_capability = self.controller.get_capability("configuration")

            if configuration_capability is not None:
                result = configuration_capability.load(config_path)
            else:
                result = self.controller.load_configuration(config_path)
            if result:
                logger.info("配置文件加载成功")
    ...
    def _start_measurement(self, task: Task | ExecutionPlan):
        ...
        try:
            measurement_capability = None
            if hasattr(self.controller, "get_capability"):
                measurement_capability = self.controller.get_capability("measurement")

            if measurement_capability is not None:
                success = measurement_capability.start()
            else:
                success = self.controller.start_test()
    ...
    def _start_test_execution(self, task: Task | ExecutionPlan):
        ...
        try:
            ...
            if test_cases:
                ...
                wait_ok = (
                    rpc_capability.wait_for_complete(timeout=self._task_timeout(task))
                    if rpc_capability is not None
                    else self.controller.wait_for_test_complete(timeout=self._task_timeout(task))
                )
                if not wait_ok:
    ...
    def _collect_tsmaster_results(self, task: Task | ExecutionPlan) -> list:
        ...
        try:
            ...
            if hasattr(self.controller, "get_capability"):
                artifact_capability = self.controller.get_capability("artifact")
